...
- **(d) Question focus view — full-surface takeover.** The enlarge affordance opens a full-viewport `role="dialog"` (aria-modal, labelled "Question N of M") that REPLACES the worksheet (not a centered scrim modal, §1) so a single question can be enlarged for readability. Type is rem-based so browser zoom compounds; the figure scales up. Keyboard: ← → navigate, S = solution, Esc = close; focus moves in on open and returns to the originating row on close; all interactive elements get focus indicators ≥2px at ≥3:1 contrast (outline + box-shadow). Answer recording uses the one shared result path; works across core AND extra (the extra solution lock applies).
